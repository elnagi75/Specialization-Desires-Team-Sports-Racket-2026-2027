...
# 7. قراءة البيانات بأمان (تحويل كل شيء لنص لمنع الأخطاء)
...
def_score = student_data.iloc[0]["مجموع درجات بالأرقام"] if not student_data.empty else ""
...
# 9. القسم الثاني: الرغبات (في المنتصف)
...
with center_col:
    ...
    def get_index(val, options):
        return options.index(val) + 1 if val in options else 0

    p1_val = student_data.iloc[0]["(1) الرغبة الأولى:"] if not student_data.empty else ""
    pref1 = st.selectbox("الرغبة الأولى:", ["اختر التخصص..."] + all_sports, index=get_index(p1_val, all_sports))

    options2 = ["اختر التخصص..."] + [s for s in all_sports if s != pref1]
    p2_val = student_data.iloc[0]["(2) الرغبة الثانية:"] if not student_data.empty else ""
    pref2 = st.selectbox("الرغبة الثانية:", options2, index=get_index(p2_val, options2), disabled=(pref1 == "اختر التخصص..."))

    options3 = ["اختر التخصص..."] + [s for s in all_sports if s not in [pref1, pref2]]
    p3_val = student_data.iloc[0]["(3) الرغبة الثالثة:"] if not student_data.empty else ""
    pref3 = st.selectbox("الرغبة الثالثة:", options3, index=get_index(p3_val, options3), disabled=(pref2 == "اختر التخصص..."))

    options4 = ["اختر التخصص..."] + [s for s in all_sports if s not in [pref1, pref2, pref3]]
    p4_val = student_data.iloc[0]["(4) الرغبة الرابعة:"] if not student_data.empty else ""
    pref4 = st.selectbox("الرغبة الرابعة:", options4, index=get_index(p4_val, options4), disabled=(pref3 == "اختر التخصص..."))

    options5 = ["اختر التخصص..."] + [s for s in all_sports if s not in [pref1, pref2, pref3, pref4]]
    p5_val = student_data.iloc[0]["(5) الرغبة الخامسة والأخيرة:"] if not student_data.empty else ""
    pref5 = st.selectbox("الرغبة الخامسة والأخيرة:", options5, index=get_index(p5_val, options5), disabled=(pref4 == "اختر التخصص..."))

st.markdown("<br>", unsafe_allow_html=True)

# 10. الاعتماد والحفظ
if st.button("إرسال واعتماد الرغبات نهائياً"):
    if not name or not national_id or not score or not whatsapp or pref1 == "اختر التخصص..." or pref5 == "اختر التخصص...":
        ...
        if str(national_id) in df["الرقم القومي"].values:
            ...
            df.loc[idx, "مجموع درجات بالأرقام"] = str(score)
            ...
            new_row = pd.DataFrame([{
                ...
                "مجموع درجات بالأرقام": str(score),
