...
with col2: st.markdown('<div style="text-align: center; font-weight: bold; color: #1E3A8A; font-size: 24px;">جامعة المنيا - كلية علوم الرياضة<br>منصة تسجيل رغبات التخصصات (2026-2027)</div>', unsafe_allow_html=True)

# جلب البيانات في حال وجود الرقم القومي
...
with col_right:
    ...
    national_id_input = national_id # تم استخدامه أعلاه
with col_left:
...
if st.button("إرسال واعتماد الرغبات"):
    if not name or not national_id or not score or not whatsapp: st.error("يرجى استكمال البيانات.")
    else:
